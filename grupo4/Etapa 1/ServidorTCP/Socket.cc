...
/**
  *  Class constructor
  *     builds an instance from an already existing descriptor
  *     (used by AcceptConnection, after "accept")
  *
...
Socket::Socket( int id ) {
   ...
   this->Init( id );      // Call base class constructor (VSocket::Init(int))
...
size_t Socket::Write( const char * text ) {
   ...
   st = this->Write( text, strlen( text ) );
   if ( -1 == st ) {
