...
class SSLSocket : public VSocket {
   ...
   public:
      ...
      VSocket * CompletarConexion( int );
      void UseCertificate( const char *, const char * );	// permite que un cliente cargue y presente su propio certificado

   private:
      void InitSSL( bool = false );		// Defaults to create a client context, true if server context needed
      void InitContext( bool );
      void LoadCertificates( const char *, const char *, bool requirePeerCert = false );
